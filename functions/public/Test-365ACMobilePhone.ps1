...
.DESCRIPTION
The Test-365ACMobilePhone function checks if users in Microsoft 365 have a mobile phone number and optionally validates these numbers against a provided list of valid numbers. It supports outputting the results to an Excel file, an HTML file, a Markdown file, or the console.
...
function Test-365ACMobilePhone {
    ...
    BEGIN {
        ...
        if ($ValidationExcelFilePath) {
            ...
            $validMobilePhones = Import-Excel -Path $ValidationExcelFilePath
        }
        if ($InteractiveLogin) {
    ...
    PROCESS {
        foreach ($user in $Users) {
            ...
            if ($user.MobilePhone) {
                if ($ValidationExcelFilePath) {
                    $hasValidMobilePhone = $validMobilePhones -contains $user.MobilePhone
                } else {
            ...
            $result = [PSCustomObject]@{
                ...
                $TestedProperty     = $hasValidMobilePhone
            }
            $results += $result
        }
    }
